...
from config import (
    ...
    INLINE_BACK_DATA, INLINE_BACK_KEYBOARD, LOGO_STEP_KEYBOARD, SKIP_LOGO_DATA,
...
async def start_custom_qr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("First, choose a color from the panel below.", reply_markup=COLOR_REPLY_MARKUP)
    return GET_COLOR
...
async def get_logo_and_prompt_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.effective_message.media_group_id:
        await update.message.reply_text("Please send only one image for the logo at a time.", reply_markup=LOGO_STEP_KEYBOARD)
        return GET_LOGO
    ...
    if not file_id:
        await update.message.reply_text("Invalid image input. Please send a photo or a valid PNG/JPEG file.", reply_markup=LOGO_STEP_KEYBOARD)
        return GET_LOGO
...
async def skip_logo_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles the 'Skip →' inline button press in the logo step."""
    query = update.callback_query
    await query.answer()

    context.user_data['logo'] = None
    logger.info(f"User {query.from_user.id} skipped logo via button.")
...
async def handle_new_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the 'Start New QR' button press by directly restarting the flow."""
    query = update.callback_query
    await query.answer()
    ...
    await query.message.reply_text("Starting new QR...", reply_markup=ReplyKeyboardRemove())
    await query.message.reply_text("First, choose a color from the panel below.", reply_markup=COLOR_REPLY_MARKUP)
    return GET_COLOR
...
async def handle_restart_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text and update.message.text.lower() == RESTART_BUTTON_TEXT.lower():
        return await start_custom_qr(update, context)

    if update.message.text and update.message.text.lower().startswith('/'):
        return
